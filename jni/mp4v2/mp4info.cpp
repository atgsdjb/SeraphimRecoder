	...
	MP4TrackId trackId)
{
	...
	if (media_data_name == NULL) {
	  ...
	  case MP4_MPEG4_AUDIO_TYPE:  {
		...
		typeName = "MPEG-4 Unknown Profile";
			...
			}else
			{
				...
				index=((pAacConfig[0]&0x07)<<1)|pAacConfig[1]>>7;
				frequency=Sampling_Frequency_Index[index];
				index=(((pAacConfig[1]&0x78)<<1)>>4);
				if (index==1)
				{
					channel=1;
				}
	  ...
	  default:
		...
		foundType = true;
		frequency=MP4GetTrackTimeScale(mp4File, trackId);;
		break;
	      }
	    }
	  }
	...
	if (foundType)
	  sprintf(sInfo,
		  "%u\taudio\t%s%s, %.3f secs, %u kbps, %u Hz %d channel\n",
		  trackId,
