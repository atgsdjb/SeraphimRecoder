	...
	bool* pIsSyncSample)
{
	...
	*pNumBytes = sampleSize;
	...
	VERBOSE_READ_SAMPLE(m_pFile->GetVerbosity(),
		printf("ReadSample: track %u id %u offset %llx size %u (0x%x)\n",
	...
	try { 
		...
		if (pStartTime || pDuration) {
			...
			VERBOSE_READ_SAMPLE(m_pFile->GetVerbosity(),
				printf("ReadSample:  start %lld duration %lld\n",
...
void MP4Track::WriteChunkBuffer()
{
	...
	VERBOSE_WRITE_SAMPLE(m_pFile->GetVerbosity(),
		printf("WriteChunk: track %u offset %llx size %u (0x%x) numSamples %u\n",
	...
	u_int8_t** ppChunk, u_int32_t* pChunkSize)
{
	...
	*ppChunk = (u_int8_t*)MP4Malloc(*pChunkSize);
	...
	VERBOSE_READ_SAMPLE(m_pFile->GetVerbosity(),
		printf("ReadChunk: track %u id %u offset %llx size %u (0x%x)\n",
	...
	u_int8_t* pChunk, u_int32_t chunkSize)
{
	...
	VERBOSE_WRITE_SAMPLE(m_pFile->GetVerbosity(),
		printf("RewriteChunk: track %u id %u offset %llx size %u (0x%x)\n",
